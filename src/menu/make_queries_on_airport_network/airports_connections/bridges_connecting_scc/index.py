from src.database.session.index import SessionDB
from src.graph.airports import Airports
from src.graph.airports_connections import AirportsConnections
from src.graph.graph import AirportGraph
from src.graph.visualize.graph_visualize_arc import GraphVisualizeARC


def display_bridges_connecting_scc():

    airports: Airports = SessionDB.get("airports")
    connections: AirportsConnections = SessionDB.get("connections")

    graph = AirportGraph(airports, connections)
    connections_between_scc = graph.connections_between_strongly_connected_components()

    for from_airport_id, connected_airports in connections_between_scc.get_airports_connections().items():
        for to_airport_id in connected_airports:
            print(f"{from_airport_id} -> {to_airport_id}")

    visualize = input("Visualize connections between strongly connected components (t/n): ")

    if visualize == "t":
        GraphVisualizeARC.visualize_airports_connections(airports, connections_between_scc)

